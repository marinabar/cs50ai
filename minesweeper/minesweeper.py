...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # print(Sentence(new_cells, count))
        ...
        for i in range(len(self.knowledge)):
            for j in range(i + 1, len(self.knowledge)):
                ss1 = self.knowledge[i]
                ss2 = self.knowledge[j]
                # check entailment of set1 in set2
                if ss1.cells.issubset(ss2.cells):
                    set_cells = ss2.cells - ss1.cells
                    s = Sentence(set_cells, ss2.count - ss1.count)
                    # check if not in KN
                    if s not in self.knowledge:
                        self.knowledge.append(s)
                # check entailment of set2 in set1
                elif ss2.cells.issubset(ss1.cells):
                    set_cells = ss1.cells - ss2.cells
                    s = Sentence(set_cells, ss1.count - ss2.count)
                    # check if not in KN
                    if s not in self.knowledge:
                        self.knowledge.append(s)
