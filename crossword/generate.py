...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for var in self.domains:
            ...
            for word in words:
                if len(word) != var.length:  # If the length of the word does not correspond to the length of
                    # the available cases
    ...
    def revise(self, x, y):
        ...
        if overlap is None:  # Check if there is overlap
            return False

        revise = False # Set revision to false at first

        xwords = self.domains[x].copy()
        for word in xwords:
            notconsistent = False
            for yword in self.domains[y]:
                if word[overlap[0]] == yword[overlap[1]]:
                    notconsistent = True
                    break
            if not notconsistent:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for variable in self.crossword.variables:
            if variable not in assignment.keys():
                return variable  # Return highest degree
